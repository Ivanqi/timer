...
class sort_timer_lst 
{
    public:
         ...
         */
        void adjust_time(util_timer* timer)
        {
            if (!timer) return;
            ...
            /**
             * 如果被调整的目标定时器处在链表尾部，或者该定时器的超时值仍然小于其下一个定时器的超时值，则不用调整
             */
            ...
            if (timer == head) {
                ...
                /**
                 * 如果该定时器不是链表的头节点，则将该定时器从链表中取出，然后插入其原来的位置之后的部分链表中
                 */
        ...
        void tick() 
        {
            ...
            while (tmp) {
                ...
                // 执行完定时器中的定时任务之后，就将它从链表中删除，并重置表头节点
                head = tmp->next;
                if (head) {
                    head->prev = NULL;
                }
    ...
    private:
        /**
         * 一个重载的辅助函数，它被公有的add_timer函数和 adjust_timer函数调用
        ...
        void add_timer(util_timer* timer, util_timer* lst_head)
        {
            ...
            /**
             * 遍历lst_head节点之后的部分链表，直到找到一个超时时间大于目标的超时时间的节点，并将目标定时器插入该节点之前
             */
            while (tmp) {
            ...
            /**
             * 如果遍历完lst_head 节点之后的部分链表，仍未找到超时时间大于目标定时器的超时时间的节点
             * 则将目标定时器插入链表尾部，并把它设置未链表新的尾节点
             */
            if (!tmp) {
                prev->next = timer;
                timer->prev = prev;
