...
class time_wheel
{
    public:
        ...
        // 根据定时值timeout 创建一个定时器，并把把插入合适的槽中
        tw_timer* add_timer(int timeout)
        {
            ...
            /**
             * 下面根据待插入定时器的超时值计算它待在时间轮转动多少个滴答后被触发，并将该滴答存储于变量tick中
             * 如果待插入定时器的超时值小于时间轮的槽间隔SI，则将ticks向上折合为1，否则就将ticks向下折合为 timeout / SI
             */
            if (timeout < SI) {
                ticks = 1;
            } else {
            ...
            if (!slots[ts]) {
                printf("add timer, rotation is %d , ts is %d, cur_solt is %d \n", rotation, ts, cur_slot);
                slots[ts] = timer;
            } else {
                // 否则，将定时器插入ts个槽中
                timer->next = slots[ts];
